...
function httpGet(path, callback)
{
	...
	console.log (path);
	var req = http.request(path, onDataCallback);
...
RequestProcessor = function(action, query, response) {
	...
	this.sendConfig = function(response) {
		...
		var config = {
			...
			exchanges: [
				...
				{value: "ASX", name: "Australia", desc: "Australia"},
				{value: "BKK", name: "Thai Stock", desc: "Thai Stock"}
			],
			symbolsTypes: [
	...
	this.sendSymbolInfo = function(symbolName, response) {
		...
		var address = "/instrument/1.0/" + encodeURIComponent(symbolInfo.name) + "/chartdata;type=quote;range=1m/json";
		var that = this;
		...
		httpGet({
			...
			var lastPrice = _lastYahooResponse["previous_close_price"] + "";
	...
	this.sendSymbolHistory = function(symbol, startDateTimestamp, endDateTimestamp, resolution, response) {
		...
		var address = "/table.csv?s=" + symbolInfo.name +
			"&a=" + month +
			"&b=" + day  +
			"&c=" + year + endtext +
			"&g=" + resolution +
